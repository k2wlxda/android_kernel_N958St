...
{
    LOG_ERROR("enter\n");
    gpio_set_value(lcd_avdd_dev->vsp_en_gpio, 1);
    msleep(vsp_vsn_delay);  
    gpio_set_value(lcd_avdd_dev->vsn_en_gpio, 1);
...
void lcd_avdd_off(int vsp_vsn_delay, int avdd_off_delay)
{
    ...
    gpio_set_value(lcd_avdd_dev->vsn_en_gpio, 0);  
    msleep(vsp_vsn_delay);  
    gpio_set_value(lcd_avdd_dev->vsp_en_gpio, 0);
    LOG_ERROR("exit\n");
}
...
static int lcd_avdd_gpio_init(struct lcd_avdd_data *chip)
{
    ...
    if (gpio_is_valid(chip->vsp_en_gpio))
    {
        ...
        {
            gpio_direction_output(chip->vsp_en_gpio, 0);
            gpio_set_value(chip->vsp_en_gpio, 1);
        }
    }
    ...
    if (gpio_is_valid(chip->vsn_en_gpio))
    {
        ...
        {        
            gpio_direction_output(chip->vsn_en_gpio, 0);
            gpio_set_value(chip->vsn_en_gpio, 1);
        }
    }
    LOG_INFO("success\n");
