...
#define GOODIX_PINCTRL_STATE_SLEEP "gt9xx_int_sleep"
...
Function:
    Reset IC in Resume.
Add by luochangyang 2014.11.03.
*******************************************************/
void gtp_wakeup_reset(struct i2c_client *client)
{
    GTP_INFO("Wakeup Reset");
    GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);   // begin select I2C slave addr
    mdelay(1);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    GTP_GPIO_OUTPUT(GTP_INT_PORT, client->addr == 0x14);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /*ZTEMT Added by luochangyang, 2014/01/08*/
		...
		rst_number = of_get_named_gpio_flags(np, "goodix,reset-gpio",
				0, NULL);
		//printk("--------pdata->reset_gpio------------: %ud\n",rst_number);
	if (rst_number < 0)
		return rst_number;

		int_number = of_get_named_gpio_flags(np, "goodix,irq-gpio",
				0, NULL);
