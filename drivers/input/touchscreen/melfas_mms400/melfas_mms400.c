	...
	unsigned int write_len, char *read_buf, unsigned int read_len)
{
	...
	while(retry--){
		...
		if(res == ARRAY_SIZE(msg)){
			...
			dev_err(&info->client->dev, "%s [ERROR] i2c_transfer - size[%d] \
	...
	int start_idx, unsigned int read_len)
{
	...
	while(retry--){
		...
		if(res == read_len){
			...
			dev_err(&info->client->dev, "%s [ERROR] i2c_master_recv \
				- errno [%d]\n", __func__, res);
		}
		else if(res != read_len){
			dev_err(&info->client->dev, "%s [ERROR] length mismatch - read[%d] \
...
int mms_i2c_write(struct mms_ts_info *info, char *write_buf, unsigned int write_len)
{
	...
	while(retry--){
		...
		if(res == write_len){
			...
			dev_err(&info->client->dev, "%s [ERROR] length mismatch - write[%d] \
...
int mms_enable(struct mms_ts_info *info)
{
	...
	enable_irq(info->client->irq);//½ûÖ¹¶à´Îenable
	info->enabled = true;
...
static int mms_init_config(struct mms_ts_info *info)
{
	...
	dev_info(&info->client->dev, "%s - F/W Version : %02X.%02X %02X.%02X %02X.%02X %02X.%02X\n",
		__func__, info->fw_version[0], info->fw_version[1], info->fw_version[2],
...
static int mms_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    mms_pinctrl_init(info);

	mms_pinctrl_enable(info, 1);
