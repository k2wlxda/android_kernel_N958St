		...
		struct device_attribute *attr, char *buf)
{
    ...
    index = 0;
    cyttsp4_ic_grpdata_store(dev, attr, "0x00,0x02,0x03,0x00", 20);
    cyttsp4_ic_grpdata_store(dev, attr, "0x07,0x03,0x00", 15);
    index = cyttsp4_ic_grpdata_show(dev, attr, buf);
    dev_vdbg(dev, "%s: %s index=%d\n", __func__, buf, index);

    retval = cyttsp4_ic_grpdetect_judge(dev, 0, 3);
    if (retval == 1) {
        dev_err(dev, "%s: Grpdata is open.\n", __func__);
        goto exit;
    }

    /*3. Send Null command to set status size to 3*/
    index = 0;
    cyttsp4_ic_grpdata_store(dev, attr, "0x00,0x02,0x03,0x00", 20);
    cyttsp4_ic_grpdata_store(dev, attr, "0x07,0x04,0x00", 15);
    index = cyttsp4_ic_grpdata_show(dev, attr, buf);
    dev_vdbg(dev, "%s: %s index=%d\n", __func__, buf, index);

    retval = cyttsp4_ic_grpdetect_judge(dev, 0, 3);
    if (retval == 1) {
        dev_err(dev, "%s: Grpdata is short.\n", __func__);
        goto exit;
    }
...
exit:
    /*4. Send Null command to do command handshake*/
    index = 0;
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_dbg(dev, "%s: _cyttsp4_ret_scan_data_cmd(): returned_element:%d\n",
	...
	dev_dbg(dev, "%s: _cyttsp4_ret_scan_data_cmd(): read_byte:%d\n",
		__func__, read_byte);

	if (read_byte >= 200) {
		rc = cyttsp4_read(dad->ttsp, CY_MODE_CAT, 0, dad->ic_buf,
		                 200+element_start_offset);
		left_over_element = dad->heatmap.num_element - 100;
		read_element_offset = 100;
		data_idx = 200+element_start_offset;
         } else {
		rc = cyttsp4_read(dad->ttsp, CY_MODE_CAT, 0, dad->ic_buf,
		                 read_byte);
	...
	//left_over_element = dad->heatmap.num_element - read_byte/2;
	...
	while (left_over_element > 0) {
		...
		dev_dbg(dev, "%s: _cyttsp4_ret_scan_data_cmd(): read_byte:%d, read_element_offset:%d\n",
			__func__, read_byte, read_element_offset);
