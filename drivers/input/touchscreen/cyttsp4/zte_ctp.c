...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_TMA445
/* cyttsp */
...
#define CYTTSP4_I2C_IRQ_GPIO 915 /* J6.9, C19, GPMC_AD14/GPIO_38 */
#define CYTTSP4_I2C_RST_GPIO 914 /* J6.10, D18, GPMC_AD13/GPIO_37 */
#endif
...
#ifdef CONFIG_CYTTSP4_5P5_INCH_OTG_GW //goworld 5.5 inch NE505
#define CY_MAXX 720
#define CY_MAXY 1080
#define CYTTSP4_FIRMEARE_VERSION    0x0B27
#endif
		...
		int on, struct device *dev)
{
	...
	if(!gpio_is_valid(rst_gpio))
    	...
    	if (rc < 0) 
    	{
        printk("Failed to request GPIO:%d, ERRNO:%d", (s32)rst_gpio, rc);
        rc = -ENODEV;
    	}
   	 else
   	 {
	...
	if (rc < 0) {
			printk("Failed request CYTTSP4_I2C_IRQ_GPIO.\n");
			return rc;
		}
