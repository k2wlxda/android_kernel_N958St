...
struct cyttsp4_pinctrl_info {
	struct pinctrl *pinctrl;
	struct pinctrl_state *gpio_state_active;
	struct pinctrl_state *gpio_state_suspend;
};

static struct cyttsp4_pinctrl_info cyttsp4_pctrl;
static int cyttsp4_pinctrl_init(struct device *dev)
{
	cyttsp4_pctrl.pinctrl = devm_pinctrl_get(dev);

	if (IS_ERR_OR_NULL(cyttsp4_pctrl.pinctrl)) {
		pr_err("%s:%d Getting pinctrl handle failed\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	cyttsp4_pctrl.gpio_state_active = pinctrl_lookup_state(
					       cyttsp4_pctrl.pinctrl,
					       CY_PINCTRL_STATE_DEFAULT);

	if (IS_ERR_OR_NULL(cyttsp4_pctrl.gpio_state_active)) {
		pr_err("%s:%d Failed to get the active state pinctrl handle\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	cyttsp4_pctrl.gpio_state_suspend = pinctrl_lookup_state(
						cyttsp4_pctrl.pinctrl,
						CY_PINCTRL_STATE_SLEEP);

	if (IS_ERR_OR_NULL(cyttsp4_pctrl.gpio_state_suspend)) {
...
/*** ZTEMT Added by luochangyang, 2013/03/27 ***/
	...
	cyttsp4_pinctrl_init(&client->dev);
	rc = pinctrl_select_state(cyttsp4_pctrl.pinctrl,
			cyttsp4_pctrl.gpio_state_active);
	if (rc)
		pr_err("%s:%d cannot set pin to gpio_state_active state",
			__func__, __LINE__);
