...
static void i2c_msm_dbg_xfer_dump(struct i2c_msm_ctrl *ctrl)
{
	...
	dev_info(ctrl->dev,
		...
		"ff-tx-bc:%zu ff-rx-bc:%zu, slv_add=0x%02x\n",
		...
		struct i2c_msm_prof_event *event, size_t msec, size_t usec)
{
	...
	xfer = &ctrl->xfer;
	dev_info(ctrl->dev,
		"%3zu.%03zums  DONE_OK timeout-used:%umsec time_left:%umsec, slv_addr=0x%02x, reg=0x%02x\n",
		msec, usec, jiffies_to_msecs(event->data0),
		jiffies_to_msecs(event->data1), xfer->msgs->addr, xfer->msgs->buf[0]);
}
		...
		struct i2c_msm_prof_event *event, size_t msec, size_t usec)
{
	...
	dev_info(ctrl->dev,
		...
		"Check GPIOs configuration, slv_addr=0x%02x, reg=0x%02x\n",
		msec, usec, jiffies_to_msecs(event->data0), xfer->msgs->addr, xfer->msgs->buf[0]);
}
