...
#define MIN_ALS_POLL_DELAY_NS	60000000
	...
	#define STK_LT_POCKET       400
	#define STK_HT_POCKET 		450
...
{
	alscode += ((alscode << 7) + (alscode << 3) + (alscode >> 1));//1+128+8+0.5=137.5
    alscode <<= 3;//137.5*8=1100
...
static int32_t stk3x1x_init_all_reg(struct stk3x1x_data *ps_data)
{
    ...
    ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_STATE_REG, ps_data->state_reg);//standby
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    }
    ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_PSCTRL_REG, ps_data->psctrl_reg);//IT_PS=0.37ms,PS_GAIN=64,PS_PERS=1
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    }
    ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_ALSCTRL_REG, ps_data->alsctrl_reg);//IT_ALS=47.36ms,ALS_GAIN=64,ALS_PERS=1
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    }
    ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_LEDCTRL_REG, ps_data->ledctrl_reg);//LED_ON=IT_PS
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
    }
    ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT_REG, ps_data->wait_reg);//8*5.93=47.44ms
    if (ret < 0)
    {
        printk(KERN_ERR "%s: write i2c error\n", __func__);
        return ret;
...
{
	unsigned char value[2] = { 0 }, pid_msb = 0;
	int err = 0;
...
static int32_t stk3x1x_software_reset(struct stk3x1x_data *ps_data)
{
    ...
    w_reg = 0x7F;
    r = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT_REG, w_reg);//5.93*(128+1)
    if (r < 0)
    {
...
{
    int32_t ret = 0;
	uint8_t w_state_reg = 0, gsctrl_reg = 0;
	uint8_t org_mode = 0;
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
	...
	if(ps_data->first_boot == true)//useless
	{
    ...
    if(enable)
	{
		...
		if(!validate_reg)
		{
			...
			input_report_rel(ps_data->ps_input_dev, REL_RX, 10);
...
#endif /* #ifdef STK_CHK_REG */
		{
			...
			input_report_rel(ps_data->ps_input_dev, REL_RX, (near_far_state == 1) ? 10 : 3);
			input_sync(ps_data->ps_input_dev);
			wake_lock_timeout(&ps_data->ps_wakelock, 3 * HZ);
			reading = stk3x1x_get_ps_reading(ps_data);
		...
		unsigned int delay_msec)
{
	...
	als_data->als_poll_delay = ns_to_ktime(delay_msec * 1000000);
	return 0;
}
#endif

static ssize_t stk_als_light_check_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%s", "stk3311");
}
...
static ssize_t stk_als_lux_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	input_report_rel(ps_data->als_input_dev, REL_X, value > 0 ? value : 1);
...
static ssize_t stk_als_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	value = value * 1000000;//convert ms to ns
...
static ssize_t stk_chip_name_show(struct device *dev,
		struct device_attribute *attr,	char *buf)
{
	return sprintf(buf, "%s", "stk3311");
}

static ssize_t stk_prox_check_show(struct device *dev,
		struct device_attribute *attr,	char *buf)
{
    return sprintf(buf, "%s", "stk3311");
}
...
static ssize_t stk_ps_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    printk(KERN_ERR "%s, stk3x1x_get_state = %d\n", __func__, ret);
	if(ret < 0)
		return ret;
...
static ssize_t stk_ps_distance_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	input_report_rel(ps_data->ps_input_dev, REL_RX, (dist == 1) ? 10 : 3);
	input_sync(ps_data->ps_input_dev);
	wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);
...
static ssize_t stk_ps_distance_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	input_report_rel(ps_data->ps_input_dev, REL_RX, (value == 1) ? 10 : 3);
	input_sync(ps_data->ps_input_dev);
	wake_lock_timeout(&ps_data->ps_wakelock, 3*HZ);
...
static struct device_attribute als_light_check_attribute = __ATTR(light_check, 0444, stk_als_light_check_show, NULL);
...
{
	&als_light_check_attribute.attr,
...
static struct device_attribute ps_chip_name_attribute = __ATTR(chip_name, 0444, stk_chip_name_show, NULL);
static struct device_attribute ps_prox_check_attribute = __ATTR(prox_check, 0444, stk_prox_check_show, NULL);
...
{
	&ps_chip_name_attribute.attr,
	&ps_prox_check_attribute.attr,
...
static int stk_ps_tune_zero_final(struct stk3x1x_data *ps_data)
{
...
#ifdef CALI_PS_EVERY_TIME
	if((ps_data->ps_stat_data[1] > STK_LT_DEF) || (data < 8))
	{
		ps_data->ps_high_thd_boot = STK_HT_DEF;
		ps_data->ps_low_thd_boot = STK_LT_DEF;
		ps_data->boot_cali = 0;
	...
	{
		ps_data->ps_high_thd_boot = ps_data->ps_stat_data[1] + STK_HT_POCKET;
		ps_data->ps_low_thd_boot = ps_data->ps_stat_data[1] + STK_LT_POCKET;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_data *ps_data)
{
	...
	if((ps_data->ps_nf == 0) && (ps_data->psi_set != 0) && (ps_data->psi_set != 0xFF))//??test
	{
	...
	if(ret == 0)
	{
		ret = stk3x1x_i2c_read_data(ps_data->client, STK_DATA1_PS_REG, 2, &value[0]);
		if(ret < 0)
		{
			printk(KERN_ERR "%s fail, ret=0x%x", __func__, ret);
			return ret;
	...
	printk(KERN_INFO "%s: diff=%d,psi=%d,ps_high_thd_tmp=%d,boot_ct=%d\n", __func__, 
		diff, ps_data->psi,ps_data->ps_high_thd_tmp,ps_data->boot_ct);
	...
	if(diff == ps_data->psi)
	{
		...
		if(ps_data->ps_thd_h > ps_data->ps_high_thd_tmp)
		{
			if((ps_data->boot_cali >= 1)&&(ps_data->boot_ct != 0xFFFF))
			{
				ps_data->ps_high_thd_boot = ps_data->boot_ct + STK_HT_POCKET;
				ps_data->ps_low_thd_boot = ps_data->boot_ct + STK_LT_POCKET;
		...
		else if(ps_data->psi < ps_data->boot_ct)
		{
			...
			ps_data->ps_high_thd_tmp = ps_data->boot_ct + STK_HT_POCKET;
			ps_data->ps_low_thd_tmp = ps_data->boot_ct + STK_LT_POCKET;
...
#ifdef STK_DEBUG_PRINTF
	printk(KERN_INFO "tune0 finsh %s: boot HT=%d, LT=%d, boot_cali=%d\n", 
...
static void stk_als_poll_work_func(struct work_struct *work)
{
	...
	int32_t reading = 0, reading_lux = 0, als_comperator = 0, flag_reg = 0;
	#ifdef STK_IRS
	int ret = 0;
	#endif
#ifdef STK_GES
	if(ps_data->ges_enabled)
	{
		...
		input_report_rel(ps_data->als_input_dev, REL_X, ps_data->als_lux_last > 0 ? ps_data->als_lux_last : 1);
		input_sync(ps_data->als_input_dev);
#ifdef STK_DEBUG_PRINTF
	...
	if(abs(ps_data->als_lux_last - reading_lux) >= STK_ALS_CHANGE_THD)
	{
		...
		input_report_rel(ps_data->als_input_dev, REL_X, reading_lux > 0 ? reading_lux : 1);
		input_sync(ps_data->als_input_dev);
#ifdef STK_DEBUG_PRINTF
...
static void stk_ps_poll_work_func(struct work_struct *work)
{
	...
	uint8_t disable_flag2 = 0, org_flag2_reg = 0;
...
static void stk_work_func(struct work_struct *work)
{
	...
	ps_data->ps_distance_last = near_far_state;
	//input_report_abs(ps_data->ps_input_dev, ABS_DISTANCE, near_far_state);
	input_report_rel(ps_data->ps_input_dev, REL_RX, (near_far_state == 1) ? 10 : 3);//gpio status represent near/far
	input_sync(ps_data->ps_input_dev);
	wake_lock_timeout(&ps_data->ps_wakelock, 3 * HZ);
	reading = stk3x1x_get_ps_reading(ps_data);
#ifdef STK_DEBUG_PRINTF
			...
			struct stk3x1x_platform_data *pdata)
{
	...
	rc = of_property_read_u32(np, "stk,ps-ht-n-ct", &temp_val);
	if (!rc)
		pdata->ht_n_ct = temp_val;
	else {
		dev_err(dev, "Unable to read ps-ht-n-ct\n");
		return rc;
	}

	rc = of_property_read_u32(np, "stk,ps-lt-n-ct", &temp_val);
	if (!rc)
		pdata->lt_n_ct = temp_val;
	else {
		dev_err(dev, "Unable to read ps-lt-n-ct\n");
		return rc;
	}
...
*/
    ...
    set_bit(EV_REL, ps_data->als_input_dev->evbit);
    set_bit(REL_X,  ps_data->als_input_dev->relbit);

    set_bit(EV_REL,   ps_data->ps_input_dev->evbit);
    set_bit(REL_X,    ps_data->ps_input_dev->relbit);//ps_raw_data
    set_bit(REL_Y,    ps_data->ps_input_dev->relbit);//hi
    set_bit(REL_Z,    ps_data->ps_input_dev->relbit);//lo
    set_bit(REL_RX,   ps_data->ps_input_dev->relbit);//result
    set_bit(REL_MISC, ps_data->ps_input_dev->relbit);//ps_data
    ...
    err = sysfs_create_group(&ps_data->light_dev->kobj, &stk_als_attribute_group);
	if (err < 0)
	{
		printk(KERN_ERR "%s:could not create sysfs group for als\n", __func__);
		return err;
	}
	err = sysfs_create_group(&ps_data->proximity_dev->kobj, &stk_ps_attribute_group);
	if (err < 0)
	{
		printk(KERN_ERR "%s:could not create sysfs group for ps\n", __func__);
		return err;
	...
	dev_set_drvdata(ps_data->light_dev, ps_data);
	dev_set_drvdata(ps_data->proximity_dev, ps_data);
                        ...
                        const struct i2c_device_id *id)
{
	...
	ps_data->stk_ht_n_ct = plat_data->ht_n_ct;
	ps_data->stk_lt_n_ct = plat_data->lt_n_ct;
	...
	err = IS_ERR(proximity_class);
	if (err)
    {
		proximity_class = NULL;
		dev_err(&client->dev,
			"%s: failed to create class proximity\n", __func__);
		goto err_create_prox_class;
	}

    light_class = class_create(THIS_MODULE, "light");
	err = IS_ERR(light_class);
    if (err)
    {
		light_class = NULL;
		dev_err(&client->dev,
			"%s: failed to create class light\n", __func__);
		goto err_create_light_class;
	}

    ps_data->proximity_dev = device_create(proximity_class, NULL, 0, &stk_ps_driver, "proximity");
	err = IS_ERR(ps_data->proximity_dev);
    if (err)
    {
		dev_err(&client->dev,
			"%s: failed to create device proximity\n", __func__);
        goto err_create_prox_dev;
    }

    ps_data->light_dev = device_create(light_class, NULL, 0, &stk_ps_driver, "light");
	err = IS_ERR(ps_data->light_dev);
    if (err)
    {
		dev_err(&client->dev,
			"%s: failed to create device light\n", __func__);
        goto err_create_light_dev;
    }
...
err_setup_input_device:
    device_unregister(ps_data->light_dev);
    ps_data->light_dev = NULL;
err_create_light_dev:
    device_unregister(ps_data->proximity_dev);
    ps_data->proximity_dev = NULL;
err_create_prox_dev:
    class_destroy(light_class);
err_create_light_class:
    class_destroy(proximity_class);
err_create_prox_class:
#ifdef STK_GES
