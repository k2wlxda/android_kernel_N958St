...
#ifndef CONFIG_ZTEMT_BREATH_LED_N958X
static int set_out4_gpio_value(enum aw_out_val out_val)
{
	char buf;
	int ret;
	...
	ret = aw9106b_i2c_tx_byte_data(aw9106b_data.i2c_client,0x02,buf);
...
*/
  ...
  pdata->enable_ctl_gpio= of_get_named_gpio_flags(np, "aw9106b_led,choose-gpio",
				0, &pdata->enable_ctl_gpio_flags);
	if (pdata->enable_ctl_gpio < 0)
		return pdata->enable_ctl_gpio;
#endif
	return 0;
}
#else
...
#ifdef CONFIG_ZTEMT_BREATH_LED_N958X
		aw9106b_plat_data->aw9106b_enable_pinctrl = devm_pinctrl_get(&(i2c_client->dev));
	if (IS_ERR_OR_NULL(aw9106b_plat_data->aw9106b_enable_pinctrl)) {
		retval = PTR_ERR(aw9106b_plat_data->aw9106b_enable_pinctrl);
		aw9106b_plat_data->aw9106b_enable_pinctrl = NULL;
		return retval;
	}
#endif
	aw9106b_plat_data->gpio_power_active
		= pinctrl_lookup_state(aw9106b_plat_data->aw9106b_led_pinctrl,
			"aw9106b_power_active");
...
#ifdef CONFIG_ZTEMT_BREATH_LED_N958X
	aw9106b_plat_data->gpio_enable_active
		= pinctrl_lookup_state(aw9106b_plat_data->aw9106b_enable_pinctrl,
			"aw9106b_power_active");
	if (IS_ERR_OR_NULL(aw9106b_plat_data->gpio_enable_active)) {
		retval = PTR_ERR(aw9106b_plat_data->gpio_enable_active);
		aw9106b_plat_data->aw9106b_enable_pinctrl = NULL;
		return retval;
	}

	aw9106b_plat_data->gpio_enable_supend
		= pinctrl_lookup_state(aw9106b_plat_data->aw9106b_enable_pinctrl,
			"aw9106b_power_suspend");
	if (IS_ERR_OR_NULL(aw9106b_plat_data->gpio_enable_supend)) {
		retval = PTR_ERR(aw9106b_plat_data->gpio_enable_supend);
		aw9106b_plat_data->aw9106b_enable_pinctrl = NULL;
		return retval;
	}
#endif
						...
						bool on)
{
	...
	#ifdef CONFIG_ZTEMT_BREATH_LED_N958X
	pins_state = on ? aw9106b_plat_data->gpio_enable_active
		: aw9106b_plat_data->gpio_enable_supend;
	if (!IS_ERR_OR_NULL(pins_state)) {
		ret = pinctrl_select_state(aw9106b_plat_data->aw9106b_enable_pinctrl, pins_state);
		if (ret) {
			return ret;
		}
	}
...

{
...
#ifdef CONFIG_ZTEMT_BREATH_LED_N958X
	if (gpio_is_valid(pdata->enable_ctl_gpio)) {
	ret = gpio_request(pdata->enable_ctl_gpio, "aw9106b_enable_gpio");
	if (ret) {
		dev_err(&client->dev, "enable gpio request failed");
		return ret;
	}

	ret = gpio_direction_output(pdata->enable_ctl_gpio, 1);
	if (ret) {
		dev_err(&client->dev,
			"set_direction for reset gpio failed\n");
		 return ret;
	...
	msleep(1000);
	gpio_set_value_cansleep(pdata->enable_ctl_gpio, 1);
	}
#endif
	...
	aw9106b_control_init();
#ifndef CONFIG_ZTEMT_BREATH_LED_N958X
	ret = set_out_led_gpio(4,AW_GPIO_MODE);
	if(ret < 0)
	   pr_err("%s: set out[4] gpio mode fail!\n",__func__);

	ret = set_out4_gpio_value(0x00);
	if(ret < 0)
		pr_err("%s: write out_val[0x02] fail!\n",__func__);
#endif
