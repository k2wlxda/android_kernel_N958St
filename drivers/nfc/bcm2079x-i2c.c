...
*/
	printk("[dsc][broadcom,wake-gpio]print 3 \n");
	//pdata ->irq_gpio = of_get_named_gpio_flags(np, "broadcom,irq-gpio",0, NULL);
	pdata ->irq_gpio = of_get_named_gpio(np, "broadcom,irq-gpio",0);
	printk("[dsc][broadcom,wake-gpio]print 33 \n");
/*
	pdata ->irq_gpio = of_get_named_gpio(np, "broadcom,irq-gpio",0);
	printk("[dsc][broadcom,irq-gpio]print 4 \n");
	ret = gpio_request(platform_data->irq_gpio, "nfc_int");
	if (ret){	
		printk("[dsc]fail to request irq_gpio \n");
		return -ENODEV;
		}
*/printk("[dsc][broadcom,request clk_gpio]print 44 \n");
	gpio_request(clk_gpio, "gpio");
	printk("[dsc][broadcom,request clk_gpio]print 4 \n");
    ...
    //printk("%s: clk_gpio: %d, en_gpio: %d,wake_gpio: %d,",__FUNCTION__,clk_gpio,pdata ->en_gpio,pdata ->wake_gpio);
	return 0;
}
