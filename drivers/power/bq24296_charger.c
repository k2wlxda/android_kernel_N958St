...
static int bq24296_chg_is_otg_en_set(struct bq24296_chg_chip *chip);
static int bq24296_chg_charge_en(struct bq24296_chg_chip *chip, int enable);
/********************************************************\
			 I2C I/O function 				       
\*********************************************************/
#if 0
static int __bq24296_i2c_read(struct bq24296_chg_chip *chip, int reg,
				u8 *val)
{
	s32 ret;
...
static int __bq24296_i2c_write(struct bq24296_chg_chip *chip, int reg,
						u8 val)
{
	s32 ret;
...
static int bq24296_i2c_read(struct bq24296_chg_chip *chip, int reg,
				u8 *val)
{
	int rc;
	...
	rc = __bq24296_i2c_read(chip, reg, val);
	mutex_unlock(&chip->read_write_lock);

	return rc;
}

static int bq24296_i2c_write(struct bq24296_chg_chip *chip, int reg,
						u8 val)
{
	int rc;
	...
	mutex_lock(&chip->read_write_lock);
	rc = __bq24296_i2c_write(chip, reg, val);
	mutex_unlock(&chip->read_write_lock);
...
#endif
static int bq24296_read_bytes(struct bq24296_chg_chip *chip, int reg,
						u8 *val, u8 bytes)
{
	s32 rc;
...
EXPORT_SYMBOL(bq24296_read_bytes);

static int bq24296_masked_write(struct bq24296_chg_chip *chip, int reg,
						u8 mask, u8 val)
{
	s32 rc;
	u8 temp;
	...
	mutex_lock(&chip->read_write_lock);
	rc = __bq24296_i2c_read(chip, reg, &temp);
	if (rc < 0) {
	...
	temp &= ~mask;
	temp |= val & mask;
	rc = __bq24296_i2c_write(chip, reg, temp);
	if (rc < 0) {
		dev_err(chip->dev,
				...
				int sysmin_voltage_limit_mv)
{
	...
	*/
	...
	if (!rc){
		...
		chip->cfg_charging_enabled = true;
	}
...
bq24296_chg_wd_reset(struct bq24296_chg_chip *chip, int wd_reset)
{
  ...
  host_mode = bq24296_chg_is_otg_en_set(chip);
...
bq24296_chg_charge_en(struct bq24296_chg_chip *chip, int enable)
{
	...
	if (rc){
		DBG_CHARGE("%s  i2c r/w failed: rc=%d\n", __func__, rc);
			...
			chip->cfg_charging_enabled = enable;
...
static int
bq24296_chg_is_otg_en_set(struct bq24296_chg_chip *chip)
{
	u8 usb_otg_en;
	int rc;

	rc = bq24296_i2c_read( chip,BQ24296_REG_PO_CNTL,&usb_otg_en);
	...
	DBG_CHARGE("usb otg en 0x%x\n", usb_otg_en);

	return (usb_otg_en & OTG_ENABLE) ? 1 : 0;
}
 ...
 enum bq24296_charge_status charging_status;
  ...
  if(!bq24296_is_usb_present(bq24296_chip))
	...
	if (charging_status == CHARGE_STATE_NO_CHG)
		chg_type = POWER_SUPPLY_STATUS_NOT_CHARGING;
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		val->intval = chip->cfg_charging_enabled;
	break;
...
bq24296_wd_work(struct work_struct *work)
{
    ...
    chip->batt_voltage = bq24296_get_prop_battery_voltage_now(chip);
...
int  bq24196_set_chg_status(int usb_in)
{
  	DBG_CHARGE("usb_in=%d\n",usb_in);
...
EXPORT_SYMBOL_GPL(bq24196_set_chg_status);
...
static int bq24296_otg_regulator_enable(struct regulator_dev *rdev)
{
 ...
 rc = bq24296_i2c_write(chip,BQ24296_REG_PO_CNTL,ret);
	if (!rc){
		pr_debug("%s:Write Reg 0x%02X=0x%02X\n",
										__func__,BQ24296_REG_PO_CNTL,ret);
		chip->cfg_charging_enabled = false ;
	}
	else
 ...
 rc = bq24296_i2c_write(chip,BQ24296_REG_PO_CNTL,ret);
	if (!rc){
		pr_debug("%s:Write Reg 0x%02X=0x%02X\n",
			...
			DBG_CHARGE("Couldn't disable  OTG mode rc=%d\n", rc);
	
	/*wait 20us*/
	 udelay(20);
   DBG_CHARGE("  OTG Disable Success ! \n" );
...
//Control Debug Msg
   ...
   bq24296_gpio_select(chip, true);
	...
	chip->bq24296_batt_psy.type = POWER_SUPPLY_TYPE_BATTERY;
