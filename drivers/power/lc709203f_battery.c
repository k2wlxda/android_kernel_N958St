...
#define   BATT_TEMP_HIGH   		700
#define   BATT_TEMP_LOW    		-100
...
#ifdef CONFIG_ZTEMT_BQ24296_CHARGE
	extern struct bq24296_chg_chip   *bq24296_chip;
	extern int bq24296_is_usb_present( struct bq24296_chg_chip   * bq24296_chip);
#endif
...
static int lc709203f_get_chg_status(struct lc709203f_chip* chip)
{
	...
	if (chip->batt_psy){
		chip->batt_psy->get_property(chip->batt_psy,POWER_SUPPLY_PROP_CHARGE_TYPE,&ret);
	}
	return chip->chg_status = ret.intval;
}
...
#ifdef CONFIG_ZTEMT_BQ24296_CHARGE
	if(bq24296_chip){		
		return bq24296_is_usb_present(bq24296_chip);
	}
#else
	union power_supply_propval ret = {0,};
...
static void lc709203f_update_soc_work(struct work_struct *work)
{
	...
	chg_st   = lc709203f_get_chg_status(chip);
	usb_in   = lc709203f_is_charger_online();
	...
	/*to improve the full charge battery voltage,the soc will change from 99 to 100 when charging is cutoff
	* or the soc is still 99 until the charging cutoff
	*/
	if( usb_in && (batt_soc==100) ){	
		if(new_batt_status == POWER_SUPPLY_STATUS_FULL){
			batt_soc = 100;
		}else{
			batt_soc = 99;
		}
