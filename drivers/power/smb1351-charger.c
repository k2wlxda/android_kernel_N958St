...
#ifdef CONFIG_ZTEMT_SMB1351_CHARGE
static int smbchg_debug = 1;
module_param(smbchg_debug, int, 0644);
#define smbchg_info(x...) do {if (smbchg_debug) pr_info("-smblog-  " x); } while (0)
#endif

#ifdef CONFIG_ZTEMT_BQ27520_BATTERY
extern int bq27520_report_batt_capacity(void);
#endif
...
static int smb1351_get_prop_batt_capacity(struct smb1351_charger *chip)
{
    ...
    #ifdef CONFIG_ZTEMT_BQ27520_BATTERY
	ret.intval = bq27520_report_batt_capacity();
	return ret.intval;
	#else
	if (chip->bms_psy) {
